...
contract StructAndArrays{
    ...
    function personInGroupWithID(uint _index) public view returns(People memory){
        return group[_index];
    }
    ...
    function removeFromGroupArrayWithoutChangingTheLength(uint _index) public {
        delete group[index];    // This line will not reduce the size of array
    ...
    function removeElementMethodOne(uint _index) public {
        for(uint start=index+1; start<group.length; group++){
